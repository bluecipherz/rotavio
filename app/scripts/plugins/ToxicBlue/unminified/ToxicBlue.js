...
function Blue3D(){
    ...
    function render3D(obj){
        ...
        if(obj.depth != 0){
            obj.el.css(transformMixin('rotateY('+obj.x+'deg) rotateX('+( -1 * obj.y)+'deg) translateX('+( me.X / obj.depth * -1)+'px) translateY('+( me.Y / obj.depth * -1)+'px)'));
        }else{
            obj.el.css(transformMixin('rotateY('+obj.x+'deg) rotateX('+( -1 * obj.y)+'deg)'));
        }
    }
...
function BluePages(){
    ...
    function getDelta(event) {
        e = window.event || e;
        if(e != null){
            var wDelta = e.wheelDelta < 0 ? 1 : 0;
        }else{
            var wDelta = event.deltaY < 0 ? 0 : 1;
        }
        return wDelta;
    }
